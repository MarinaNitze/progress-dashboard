...
export default function useGatsbyImages({
  ...
  const { heroImages, topicImages, featureImages } = useStaticQuery<HeroAndCardImagesQuery>(graphql`
    query HeroAndCardImages {
      ...
      featureImages: allFile(filter: { relativeDirectory: { eq: "images/features" } }) {
        edges {
          node {
            relativePath
            childImageSharp {
      ...
      topicImages: allFile(filter: { relativeDirectory: { eq: "images/topics" } }) {
        edges {
          node {
            relativePath
            extension
